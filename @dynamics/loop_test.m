function [mod,dyn,abnormal_length] = loop_test(dyn,mod,varargin)
            ip = inputParser;
...
%--------------------------------------------------------------------------
            s_max=mod.mod{i_mod(2)}.pm.l0*0.1;
%==========================================================================
%%
%          if i_step==1
...
%==========================================================================
%%
         [mod.TypForce] = var_dt(mod.TypForce,mod.mod{i_mod(2)},'f_const_only',false,'case_f',5);
...
da=zeros(mod.mod{i_mod(1)}.var.n_coord,6);
         for ic=1:mod.mod{i_mod(1)}.var.n_coord
             ...
             firstAttempt=true;
             while (firstAttempt==true)||((norm(da_tem)>s_max) || (norm(dr_tem)>s_max))
             da_tem=Qr*dt_tem+mu_r_s*randn(3,1)*sqrt(2*mod.pm.kBT*dt_tem);
             dr_tem=(Qt*dt_tem+mu_t_s*randn(3,1)*sqrt(2*mod.pm.kBT*dt_tem));
%              da_tem=Qr*dt_tem;
%              dr_tem=Qt*dt_tem;
             dt_tem=dt_tem*0.5;
             firstAttempt=false;
             ...
             dr(ic,1:3)=Qt';    dr(ic,4:6)=(mu_t_s*randn(3,1))'*mod.pm.kBT;            
             da(ic,1:3)=Qr';    da(ic,4:6)=(mu_r_s*randn(3,1))'*mod.pm.kBT;
         end
%========================================================================== 
%%
         [dyn] = get_dt(dyn);
...
%%
         for ic=1:mod.mod{i_mod(1)}.var.n_coord
             mod.mod{i_mod(1)}.var.a(ic,:)=mod.mod{i_mod(1)}.var.a(ic,:)+da(ic,1:3)*dyn.pm.dt_var+da(ic,4:6)*sqrt(2*mod.pm.kBT*dyn.pm.dt_var);
             ...
             mod.mod{i_mod(1)}.var.coord(ic,:)=mod.mod{i_mod(1)}.var.coord(ic,:)+dr(ic,1:3)*dyn.pm.dt_var+dr(ic,4:6)*sqrt(2*mod.pm.kBT*dyn.pm.dt_var); %kBT=1
         end   
...
%%
         mod.mod{i_mod(2)}.var.coord(mod.mod{i_mod(2)}.var.id_on_coord,:)=mod.mod{i_mod(2)}.var.coord(mod.mod{i_mod(2)}.var.id_on_coord,:)...
                                    +(mod.TypForce.int_const.ModMembrane(mod.mod{i_mod(2)}.var.id_on_coord,:)...
                                     ...
                                     +mod.TypForce.int_rand.ModMemAdapter_ModMembrane(mod.mod{i_mod(2)}.var.id_on_coord,:))*mod.mod{i_mod(2)}.pm.mu*dyn.pm.dt_var;
%--------------------------------------------------------------------------         
